...
async function main() {
  ...
  for (const content of destinationContent) {
    ...
    if (!existing) {
      console.warn(`  ⚠️  No Sanity document found for slug: ${content.slug} — skipping`);
      continue;
    }

    console.log(`  → Patching ${existing.title?.it ?? content.slug} (${existing._id})`);
